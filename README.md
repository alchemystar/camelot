...
1. 按 Spring 原生方式启动目标应用（默认反射调用启动类 `main(String[] args)`）。
...
- `hahaha`（如果你在启动类 `main` 里打印，说明现在默认会反射调用 `StartApp.main(args)`）
...
- `--entry=com.xxx.Service#paySync/1`：指定入口方法并自动构造 fake 参数调用（签名格式：`全限定类名#方法名/参数个数`）
- `--dot-file=/tmp/call-chain.dot`：调用链 DOT 输出文件（默认 `call-chain.dot`）
